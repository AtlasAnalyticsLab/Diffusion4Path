...
class DDPMPipeline:
    ...
    def ddpm_beta_sampling(self, model, initial_noise, device, z_values, guide_w, c_i, save_all_steps=False):
        ...
        for timestep in tqdm(range(self.num_timesteps - 1, -1, -1)):
            ...
            predicted_noise = model(image, ts, context_mask, y=c_i)
            
            eps1 = predicted_noise[:1]
            eps2 = predicted_noise[1:]
    ...
    @torch.no_grad()
    def ddpm_beta_hat_sampling(self, model, initial_noise, device, z_values,guide_w,labels, save_all_steps=False):
        """
        Algorithm 2 from the paper https://arxiv.org/pdf/2006.11239.pdf
        Seems like we have two variations of sampling algorithm: iterative and with reparametrization trick (equation 15)
        Iterative assumes you have to denoise image step-by-step on T=1000 timestamps, while the second approach lets us
    ...
    @torch.no_grad()
    def epsilon_sampling(self, model, initial_noise, device, z_values, scale, guide_w,labels,save_all_steps=False):
        """
        Algorithm 2 from the paper https://arxiv.org/pdf/2006.11239.pdf
        Seems like we have two variations of sampling algorithm: iterative and with reparametrization trick (equation 15)
        Iterative assumes you have to denoise image step-by-step on T=1000 timestamps, while the second approach lets us
        ...
        for timestep in tqdm(range(self.num_timesteps - 1, -1, -1)):
            ...
            predicted_noise = predicted_noise / scale
    ...
    @torch.no_grad()
    def improved_second_order_sampling(self, model, initial_noise, device, z_values,guide_w,labels,gamma=2,save_all_steps=False):
        """
        Algorithm 2 from the paper https://arxiv.org/pdf/2006.11239.pdf
        Seems like we have two variations of sampling algorithm: iterative and with reparametrization trick (equation 15)
        Iterative assumes you have to denoise image step-by-step on T=1000 timestamps, while the second approach lets us
        ...
        for timestep in tqdm(range(self.num_timesteps - 1, -1, -1)):
            ...
            # Noise from previous timestep
            if timestep < self.num_timesteps - 1:
                predicted_prev_noise = predicted_noise
                # if guide_w > 0:
                #     uncond_predicted_prev_noise = uncond_predicted_noise
                #     predicted_prev_noise = ((1+guide_w) * predicted_prev_noise) - (guide_w*uncond_predicted_prev_noise)
            ...
            if timestep < self.num_timesteps - 1:
                predicted_noise = (gamma * predicted_noise) + (1-gamma) * predicted_prev_noise
    ...
    @torch.no_grad()
    def adaptive_momentum_sampling(self, model, initial_noise, device, z_values, eta, steps,guide_w,labels, save_all_steps=False):
        """
        Algorithm 2 from the paper https://arxiv.org/pdf/2006.11239.pdf
        Seems like we have two variations of sampling algorithm: iterative and with reparametrization trick (equation 15)
        Iterative assumes you have to denoise image step-by-step on T=1000 timestamps, while the second approach lets us
        ...
        for timestep in tqdm(reversed(range(0, steps))):
            ...
            epsilon_theta_t = model(image, t, None, labels)
            if guide_w > 0:
                    uncond_predicted_noise = model(image, t, None, None)
                    epsilon_theta_t = ((1+guide_w) * epsilon_theta_t) - (guide_w*uncond_predicted_noise)
    ...
    @torch.no_grad()
    def momentum_sampling(self, model, initial_noise, device, z_values, eta, steps, guide_w, labels, save_all_steps=False):
        """
        Algorithm 2 from the paper https://arxiv.org/pdf/2006.11239.pdf
        Seems like we have two variations of sampling algorithm: iterative and with reparametrization trick (equation 15)
        Iterative assumes you have to denoise image step-by-step on T=1000 timestamps, while the second approach lets us
        ...
        # Define an empty list to store timestep values
        s = self.num_timesteps // steps

        # add one to get the final alpha values right (the ones from first scale to data during sampling)
        time_steps = np.asarray(list(range(0, self.num_timesteps, s)))
        time_steps = time_steps + 1
        # previous sequence
        time_steps_prev = np.concatenate([[0], time_steps[:-1]])

        b = torch.tensor(0.6, device=device)
        a = torch.tensor(0.4, device=device)
        # a = torch.sqrt(1 - b ** 2)
